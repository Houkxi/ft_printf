...
/*   Updated: 2018/02/22 13:00:43 by mmanley          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		*flag_mfield(int mfield, t_info *data, char *s)
{
	...
	else
	{
		...
		if (data->flags & PLUS_LL && s[0] != '-')
			new[mfield - ft_strlen(s) - 1] = '+';
	}
...
char		*size_testout(t_info *data, char *s, int size)
{
	...
	if (data->mfield > data->prec && data->mfield > size)
